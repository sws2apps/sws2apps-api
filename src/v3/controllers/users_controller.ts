...
export const retrieveUserBackup = async (req: Request, res: Response) => {
  ...
  const scheduleEditor = adminRole || userRole.some((role) => role === 'midweek_schedule' || role === 'weekend_schedule' || role === 'public_talk_schedule');
  ...
  if (cong.settings.data_sync.value) {
    ...
    if (localDate !== incomingDate) {
      result.app_settings.user_settings = {
        ...
        theme_follow_os_enabled: user.settings.theme_follow_os_enabled?.length > 0 ? user.settings.theme_follow_os_enabled : undefined,
...
export const saveUserBackup = async (req: Request, res: Response) => {
  ...
  const scheduleEditor = userRole.some((role) => role === 'midweek_schedule' || role === 'weekend_schedule' || role === 'public_talk_schedule');
