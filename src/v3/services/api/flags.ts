...
export const adminFlagsGet = () => {
  ...
  const result = flags.map((flag) => {
    const flagUsers = users.filter((user) =>
      user.flags.some((f) => f === flag.id),
    );
    const flagCongregations = congregations.filter((congregation) =>
      congregation.flags.some((f) => f === flag.id),
    );
