...
import { encryptData } from '../encryption/encryption-utils.js';
...
export const dbUserUpdateSessions = async (userId: string, sessions: UserSession[]) => {
	await db.collection('users').doc(userId).update({ 'about.sessions': sessions });
};

export const dbUserEnableMFA = async (userId: string) => {
	await db.collection('users').doc(userId).update({ 'about.mfaEnabled': true });

	return true;
};

export const dbUserDisableMFA = async (userId: string) => {
	await db.collection('users').doc(userId).update({ 'about.secret': FieldValue.delete(), 'about.mfaEnabled': false });
...
export const dbUserGenerateSecret = async (userId: string, user_email: string) => {
	...
	// save secret
	await db.collection('users').doc(userId).update({ 'about.secret': encryptedData });
