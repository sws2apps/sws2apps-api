...
export const loadIncomingTalks = async () => {
  for (const congregation of CongregationsList.list) {
    ...
    const approvedCongs = congregation.outgoing_speakers.access.filter(
    ...
    const affectedTalks = talks.filter(
      (record) => record.recipient === congregation.id
    );
